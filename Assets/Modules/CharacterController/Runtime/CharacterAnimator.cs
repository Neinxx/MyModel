...
#if UNITY_EDITOR
        [ContextMenu("⚡ 自动化工具/从 Animator Controller 提取切片")]
        private void ExtractClipsFromAnimator()
        {
            var anim = GetComponent<Animator>();
            if (anim == null || anim.runtimeAnimatorController == null)
            {
                Debug.LogWarning(
                    "[CharacterAnimator] 找不到 Animator 或未挂载 Animator Controller！"
                );
                return;
            }

            int count = 0;
            // 遍历状态机里的所有切片，根据命名规则自动分配
            foreach (var clip in anim.runtimeAnimatorController.animationClips)
            {
                ...
                else if (walkClip == null && lowerName.Contains("walk"))
                {
                    walkClip = clip;
                    count++;
                }
                    ...
                    slowRunClip == null
                    && (
                        lowerName.Contains("slowrun")
                        || lowerName.Contains("slow_run")
                ...
                else if (jumpClip == null && lowerName.Contains("jump"))
                {
                    jumpClip = clip;
                    count++;
                }
            ...
            if (count > 0)
            {
                ...
                Debug.Log(
                    $"<color=#3FB950><b>[CharacterAnimator]</b></color> 成功从旧的状态机中提取并自动绑定了 {count} 个动画切片！"
                );
            }
            else
            {
                Debug.Log(
                    "[CharacterAnimator] 未能匹配到合适的切片，请检查动画文件命名是否包含 idle/walk/run/jump 等关键字。"
                );
            }
        }
#endif
