...
{
    /// <summary>
    /// 工业级光环组件编辑器：支持 4 通道层级的独立预览与模块化配置
    /// </summary>
    [CustomEditor(typeof(DecalAuraComponent))]
    public class DecalAuraComponentEditor : DecalEditorBaseMini
    {
        private DecalAuraComponent _target;

        public override VisualElement CreateInspectorGUI()
        {
            _target = (DecalAuraComponent)target;
        ...
        protected override void OnGridItemClick(Texture2D tex)
        {
            ...
            var texProp = serializedObject.FindProperty("_auraModule.auraTexture");
            if (texProp != null)
            {
        ...
        private void CreateHeader(VisualElement root)
        {
            ...
            var title = new Label("Decal Aura (RGBA)");
        ...
        private void CreateModuleSection(VisualElement root)
        {
            ...
            card.Add(
                new PropertyField() { bindingPath = "sortingOrder", label = "Rendering Order" }
            );
            card.Add(
                new PropertyField()
                {
                    bindingPath = "_auraModule.auraTexture",
                    label = "RGBA Mask Atlas",
                }
            );
            ...
            settingsRow.Add(
                new PropertyField() { bindingPath = "_auraModule.softFade", label = "SOFTNESS" }
            );
            settingsRow.Add(
                new PropertyField()
                {
                    bindingPath = "_auraModule.pulseIntensity",
        ...
        private void CreateLayerSection(VisualElement root)
        {
            ...
            AddLayerRow(card, "_auraModule.layerR", "R (Red Channel)", new Color(1, 0.4f, 0.4f));
            AddLayerRow(card, "_auraModule.layerG", "G (Green Channel)", new Color(0.4f, 1, 0.4f));
            AddLayerRow(card, "_auraModule.layerB", "B (Blue Channel)", new Color(0.4f, 0.4f, 1));
            AddLayerRow(
                card,
                "_auraModule.layerA",
                "A (Alpha Channel)",
        ...
        )
        {
            ...
            // 5. 智能联动：状态同步
            strip.TrackSerializedObjectValue(
                serializedObject,
                (so) =>
                {
                    var active = so.FindProperty(path + ".active").boolValue;

                    // 修复：不要禁用整个 strip，否则开关自己也会被锁死
                    ...
                    colorBar.style.backgroundColor = active
                        ? themeColor
